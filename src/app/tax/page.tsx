...
export default function TaxPage() {
  ...
  const handleExportTXF = async () => {
    ...
    try {
      ...
      const { generateTxfExport, convertLotsToTxfTransactions } = await import("@/lib/export/txf-generator");

      // Convert lots to TXF transactions
      const txfTransactions = convertLotsToTxfTransactions(
        lots.map((lot) => ({
          amount: lot.amount,
          asset: lot.asset,
          acquisition_date: lot.acquisition_date,
          disposal_date: lot.disposal_date!,
          proceeds_usd: lot.proceeds_usd || 0,
          cost_basis_usd: lot.cost_basis_usd,
          gain_loss_usd: lot.gain_loss_usd || 0,
          is_long_term: lot.is_long_term || false,
          txid: lot.txid,
        }))
      );
      ...
      const result = generateTxfExport({
        year,
        method,
        transactions: txfTransactions,
        useSummaryMode: lots.length > 4000,
      });
      ...
      const summaryNote = result.isSummary ? " (summary mode)" : "";
      const warningNote = result.warnings.length > 0 ? ` - ${result.warnings[0]}` : "";
      setMessage({
        type: "success",
        text: `Exported ${lots.length} transactions to TXF for TurboTax Desktop${summaryNote}${warningNote}`,
      });
    } catch (err) {
      console.error("Failed to export TXF:", err);
      setMessage({ type: "error", text: "Failed to export TXF file" });
    } finally {
      setExporting(false);
    }
  };
          ...
          <>
            ...
            <div className="flex flex-wrap justify-end gap-3">
              ...
              <button
                ...
                disabled={exporting}
                className="btn-secondary disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                title="Export TXF file for TurboTax Desktop"
              >
                {exporting ? (
                  <>
                    <svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
